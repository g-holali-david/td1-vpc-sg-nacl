...
# ============================ Config ============================
REGION=eu-west-3
OWNER=david              # suffixe d'identification de TES ressources (compte partage)
KEY=cle-td               # paire de cles EC2
...
my_ip()      { echo "$(curl -s https://checkip.amazonaws.com)/32"; }
...
net_up() {
  ...
  vpc=$(vpc_id); igw=$(igw_id)
  echo "### Sous-reseau + table de routage (VPC=$vpc IGW=$igw) ###"
  subnet=$(aws ec2 create-subnet --region "$REGION" --vpc-id "$vpc" --cidr-block "$CIDR" \
  ...
  aws ec2 associate-route-table --region "$REGION" --route-table-id "$rt" --subnet-id "$subnet" >/dev/null
...
instances_up() {
  ...
  ami=$(ami_id); subnet=$(subnet_id)
  echo "### Instances (AMI=$ami subnet=$subnet) ###"
  bastion=$(aws ec2 run-instances --region "$REGION" --image-id "$ami" --instance-type "$TYPE_BASTION" \
    --key-name "$KEY" --subnet-id "$subnet" --associate-public-ip-address --count 1 \
    --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=td-bastion-$OWNER}]" \
    --query "Instances[0].InstanceId" --output text)
  cible=$(aws ec2 run-instances --region "$REGION" --image-id "$ami" --instance-type "$TYPE_CIBLE" \
    --key-name "$KEY" --subnet-id "$subnet" --no-associate-public-ip-address --count 1 \
...
sg_up() {
  ...
  aws ec2 authorize-security-group-ingress --region "$REGION" --group-id "$sgb" \
    --protocol tcp --port 22 --cidr "$ip" >/dev/null
  sgc=$(aws ec2 create-security-group --region "$REGION" \
    --group-name "td-sg-cible-$OWNER" --description "Acces depuis bastion" --vpc-id "$vpc" \
    --query GroupId --output text)
  aws ec2 authorize-security-group-ingress --region "$REGION" --group-id "$sgc" \
    --protocol tcp --port 22 --source-group "$sgb" >/dev/null
  aws ec2 authorize-security-group-ingress --region "$REGION" --group-id "$sgc" \
    --protocol icmp --port -1 --source-group "$sgb" >/dev/null
  bastion=$(instance_id "td-bastion-$OWNER"); cible=$(instance_id "td-cible-$OWNER")
...
nacl_up() {
  ...
  vpc=$(vpc_id); subnet=$(subnet_id); ip=$(my_ip)
...
info() {
  ...
  b=$(instance_id "td-bastion-$OWNER"); c=$(instance_id "td-cible-$OWNER")
  if [ -n "$b" ]; then
...
down() {
  ...
  subnet=$(subnet_id); nacl=$(nacl_id); rt=$(rt_id)
...
defense() {
  local nacl ip; nacl=$(nacl_id); ip=$(my_ip)
  [ "$nacl" = "None" ] && { echo "Aucune NACL : lance d'abord 'bash main.sh up'"; return 1; }
  ...
  aws ec2 delete-network-acl-entry --region "$REGION" --network-acl-id "$nacl" --rule-number 90 --ingress
  echo ">>> Acces retabli."
}
